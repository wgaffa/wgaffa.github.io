...
The AoC does not state what should happen if something goes wrong so I opt to 
...
transparency means is we can replace values with their implementation. Take the *elevations* part for example
...
that means the whole expression is of type `IO [Direction]`. And we then pass that down to
